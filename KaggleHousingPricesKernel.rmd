...
## Watching for potential collinearities 
...
par(mfrow=c(3,2))
hist(train$LotArea, breaks = 50, col = "navy");hist(log10(train$LotFrontage),breaks=50,col = "navy")
hist(train$MasVnrArea, breaks = 50,col = "navy");hist(log10(train$MasVnrArea),breaks=50,col = "navy")
hist(train$BsmtFinSF1, breaks = 50,col = "navy");hist(log10(train$BsmtFinSF1),breaks=50,col = "navy")
```

Indeed these features look more gaussian when they are is log10 transformed. 

```{r,fig.width=6,fig.height=6}
pairs(log10(train$SalePrice) ~ log10(train$LotArea) + log10(train$MasVnrArea) + log10(train$BsmtFinSF1),cex = 0.1, pch = 19, col = "navy")
```

When transformed, these features seem to do a better job in explain the variability in the response variable.

Therefore, we will log10 transform these 3 variables in both training and test data sets: